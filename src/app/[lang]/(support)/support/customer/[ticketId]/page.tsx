...
export default function CustomerTicketDetailPage() {
  ...
  if (!ticket) {
    return (
      <SupportLayout userRole="customer" userName={authUser?.name || tr('portal.common.customer', 'Customer')} userRoleLabel="prosERP">
    ...
    <SupportLayout userRole="customer" userName={authUser?.name || tr('portal.common.customer', 'Customer')} userRoleLabel="prosERP">
      <Box sx={{ display: 'flex', flexDirection: 'column', height: CHAT_COLUMN_HEIGHT, minHeight: 480, maxWidth: 1100 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2, flexShrink: 0 }}>
          <IconButton aria-label={t?.back || 'Back'} onClick={goBack} sx={{ color: 'var(--pc-text-3)' }}>
            <BackIcon />
          </IconButton>
