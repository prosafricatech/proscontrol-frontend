...
function PageHeader({ title, subtitle, action }: { title: string; subtitle: string; action?: React.ReactNode }) {
  return <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, gap: 2, mb: 3, flexWrap: 'wrap' }}><Box><Typography sx={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--pc-text)', mb: 0.5 }}>{title}</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.95rem' }}>{subtitle}</Typography></Box>{action}</Box>;
}

function SectionCard({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) {
  return <Card sx={cardSx}><CardContent sx={{ p: 3 }}><Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2, gap: 2 }}><Typography sx={{ fontWeight: 700, color: 'var(--pc-text)' }}>{title}</Typography>{action}</Box>{children}</CardContent></Card>;
}

function StatusPill({ status }: { status: StaffMember['status'] }) {
  const colors = { Available: ['var(--pc-success-soft)', 'var(--pc-success-strong)'], Busy: ['var(--pc-warning-soft)', 'var(--pc-warning)'], Away: ['var(--pc-surface-2)', 'var(--pc-text-3)'] };
...
function TicketRows({ tickets, onOpen }: { tickets: Ticket[]; onOpen: (id: string) => void }) {
  return <Stack divider={<Divider />} spacing={0}>{tickets.map((ticket) => <Box key={ticket.id} onClick={() => onOpen(ticket.id)} sx={{ py: 1.8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2, cursor: 'pointer', '&:hover': { bgcolor: 'var(--pc-bg)' }, px: 1, mx: -1, borderRadius: '8px' }}><Box sx={{ minWidth: 0 }}><Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 0.5 }}><StatusBadge status={ticket.status} /><Typography sx={{ color: 'var(--pc-text-4)', fontSize: '0.78rem' }}>{ticket.id}</Typography></Box><Typography sx={{ color: 'var(--pc-text)', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ticket.subject}</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.82rem' }}>{ticket.customerName} · updated {new Date(ticket.updatedAt).toLocaleDateString()}</Typography></Box><ChevronRight sx={{ color: 'var(--pc-text-4)' }} /></Box>)}</Stack>;
}

function StaffDirectory() {
  const [query, setQuery] = useState('');
  const filtered = staff.filter((member) => `${member.name} ${member.role} ${member.email}`.toLowerCase().includes(query.toLowerCase()));
  return <><PageHeader title="Staff Directory" subtitle="See who is available to help with ticket assignment and internal communication." action={<Button variant="contained" startIcon={<PersonAdd />}>Invite staff</Button>} /><TextField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search staff by name, role, or email" size="small" sx={{ ...inputSx, width: { xs: '100%', md: 360 }, mb: 2 }} InputProps={{ startAdornment: <InputAdornment position="start"><Search fontSize="small" /></InputAdornment> }} /><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 2 }}>{filtered.map((member) => <Card key={member.id} sx={cardSx}><CardContent sx={{ p: 2.5 }}><Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2 }}><Box sx={{ display: 'flex', gap: 1.5 }}><Avatar sx={{ bgcolor: 'var(--pc-accent-soft-2)', color: 'var(--pc-accent)', fontWeight: 700 }}>{member.name.charAt(0)}</Avatar><Box><Typography sx={{ fontWeight: 700, color: 'var(--pc-text)' }}>{member.name}</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.86rem' }}>{member.role}</Typography></Box></Box><StatusPill status={member.status} /></Box><Divider sx={{ my: 2 }} /><Stack spacing={1}><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.85rem' }}><Email sx={{ fontSize: 15, verticalAlign: 'middle', mr: 1 }} />{member.email}</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.85rem' }}><Assignment sx={{ fontSize: 15, verticalAlign: 'middle', mr: 1 }} />{member.load} active tickets</Typography></Stack></CardContent></Card>)}</Box></>;
}
...
function CustomerProfile() {
  ...
  return <><PageHeader title="Customer Profile" subtitle="Review customer details and their complete support history." action={<Button variant="outlined" startIcon={<Edit />}>Edit profile</Button>} /><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '320px 1fr' }, gap: 2 }}><SectionCard title="Customer"><Select fullWidth value={selectedId} onChange={(event) => setSelectedId(event.target.value)} size="small" sx={inputSx}>{supportCustomers.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</Select><Box sx={{ textAlign: 'center', py: 3 }}><Avatar sx={{ width: 72, height: 72, mx: 'auto', bgcolor: 'var(--pc-accent-soft-2)', color: 'var(--pc-accent)', fontSize: 28 }}>{customer.name.charAt(0)}</Avatar><Typography sx={{ mt: 1.5, fontWeight: 700, color: 'var(--pc-text)' }}>{customer.name}</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.85rem' }}>{customer.organization}</Typography></Box><Divider /><Stack spacing={1.5} sx={{ pt: 2 }}><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.85rem' }}><Email sx={{ fontSize: 16, verticalAlign: 'middle', mr: 1 }} />{customer.email}</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.85rem' }}><Phone sx={{ fontSize: 16, verticalAlign: 'middle', mr: 1 }} />+255 700 000 000</Typography><Typography sx={{ color: 'var(--pc-text-3)', fontSize: '0.85rem' }}><EventNote sx={{ fontSize: 16, verticalAlign: 'middle', mr: 1 }} />Customer since September 2026</Typography></Stack></SectionCard><SectionCard title="Ticket history" action={<Button size="small" startIcon={<FilterList />}>Filter</Button>}><TicketRows tickets={customerTickets} onOpen={() => {}} /></SectionCard></Box></>;
}
...
export default function SupportWorkspacePage({ view }: { view: WorkspaceView }) {
  ...
  if (staffOnlyViews.includes(view) && (authData.isLoading || !isStaff)) {
    return null;
  }
