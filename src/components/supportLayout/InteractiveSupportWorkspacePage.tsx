...
function NotificationCenter() {
  ...
  return <><Header title="Notification Center" subtitle="Ticket updates and new messages. Opening a conversation clears its message alerts." action={<Button variant="outlined" onClick={markAllRead} disabled={unreadCount === 0}>Mark all read</Button>} /><Box sx={{ display: 'flex', gap: 1, mb: 2 }}><Chip label={`All ${items.length}`} color="primary" /><Chip label={`Unread ${unreadCount}`} variant="outlined" /></Box><Card sx={cardSx}>{loading && items.length === 0 ? <Box sx={{ p: 3 }}><Typography sx={{ color: 'var(--pc-text-3)' }}>Loading notifications…</Typography></Box> : <NotificationList emptyText="No notifications yet. Updates on your tickets will appear here." />}</Card></>;
}
