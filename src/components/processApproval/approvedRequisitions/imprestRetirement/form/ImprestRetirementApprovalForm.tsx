...
function ImprestRetirementApprovalForm({
  ...
  const latestApproval =
    retirement?.latest_approval || retirement?.approval || lastApproval;
  ...
  const nextApprovalLevelIsFinal =
    Number(
      nextApprovalLevel?.is_final ?? nextApprovalLevel?.can_finalize ?? 0
    ) || 0;
  ...
  const seedItems = React.useMemo((): ApprovalItem[] => {
    ...
    return source.map((item: any) => {
      ...
      return {
        ...
        ledger_id: isProductLine
          ? null
          : Number(item?.ledger_id || item?.ledger?.id) || null,
        product_id: isProductLine
          ? Number(item?.product_id || item?.product?.id) || null
          : null,
        store_id: isProductLine
          ? Number(item?.store_id || item?.store?.id) || null
          : null,
        measurement_unit_id:
          Number(item?.measurement_unit_id || item?.measurement_unit?.id) ||
          null,
  ...
  const totalRetiredToDate = Number(
    retirement?.imprest_approval?.total_retired || 0
  );
  ...
  const { mutate: submitApproval, isPending } = useMutation({
    ...
    onSuccess: async (response: any) => {
      enqueueSnackbar(response?.message || 'Decision recorded', {
        variant: 'success',
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['imprestRetirements'] }),
        queryClient.invalidateQueries({
          queryKey: ['imprestRetirementDetails'],
        }),
        queryClient.invalidateQueries({
          queryKey: ['imprestRetirementApprovalDetails'],
        }),
      ]);
      toggleOpen(false);
    },
  ...
  const handleRemarksChange = (value: string) => {
    ...
    if (
      clientError === 'Remarks are required for this decision.' &&
      value.trim()
    ) {
      setClientError(null);
    }
  ...
  const getFieldError = (
    index: number,
    field: keyof ApprovalItemFieldErrors
  ): string => {
  ...
  return (
    <>
      <DialogTitle textAlign='center'>
        {isEdit
          ? `Edit Approval — ${retirementNo}`
          : `Approve Retirement ${retirementNo}`}
      </DialogTitle>
      ...
      <DialogContent>
          ...
          return (
            ...
            >
              ...
              <Grid
                size={{
                  xs: 12,
                  md: isProductLine ? (showStoreField ? 3.5 : 4.5) : 4.5,
                }}
              >
                {isProductLine ? (
                  <ProductSelect
                    ...
                    frontError={
                      getFieldError(index, 'source')
                        ? { message: getFieldError(index, 'source') }
                        : undefined
                    }
              ...
              <Grid size={{ xs: 12, md: 2 }}>
                <MeasurementSelector
                  label='Unit'
                  frontError={
                    getFieldError(index, 'measurement_unit_id')
                      ? { message: getFieldError(index, 'measurement_unit_id') }
                      : null
                  }
        ...
        {retirement?.id ? (
          ...
          <Alert severity='info' sx={{ mt: 1 }}>
            Supporting documents are unavailable because retirement reference is
            missing.
          </Alert>
        )}
      </DialogContent>
