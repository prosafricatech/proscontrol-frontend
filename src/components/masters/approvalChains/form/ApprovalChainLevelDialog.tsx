...
const getValidationSchema = (
  approvalChain: ApprovalChainLevelDialogProps['approvalChain']
) =>
  yup.object({
    ...
    approval_chain_id: yup.number().when([], {
      is: () => !!approvalChain,
      then: (schema) =>
        schema
          .required('Approval chain is required')
          .typeError('Approval chain is required'),
...
function ApprovalChainLevelDialog({
  ...
  const [canFinalize, setCanFinalize] = useState(
  ...
  const positionIndexOptions: PositionIndexOption[] = useMemo(
    () => [
      { label: 'At the beginning', position_index: 0, id: null },
  ...
  useEffect(() => {
    if (approvalChain?.id) {
      setValue('approval_chain_id', approvalChain.id, {
        shouldDirty: false,
        shouldValidate: true,
      });
    }
              ...
              <Div sx={{ mt: 1 }}>
                <Autocomplete
                  options={positionIndexOptions.filter(
                    (opt) =>
                      opt.position_index !== approvalChainLevel?.position_index
                  )}
                  isOptionEqualToValue={(option, value) =>
                    option.id === value.id
                  }
