...
const StakeholderDialogForm: React.FC<StakeholderDialogFormProps> = ({
  stakeholder,
  toggleOpen,
}) => {
  ...
  const validationSchema = yup
    .object({
      name: yup
        .string()
        .required(dictionary.stakeholders.form.errors.validation.name.required),
      type: yup
        .string()
        .required(dictionary.stakeholders.form.errors.validation.type.required)
        .typeError(dictionary.stakeholders.form.errors.validation.typeError),
      email: yup
        .string()
        .email(dictionary.stakeholders.form.errors.validation.email)
        .nullable(),
      website: yup
        .string()
        .matches(
    ...
    .test('at-least-one-button', 'Select one ledger type', function (value) {
      ...
      if (!create_receivable && !create_payable && !stakeholder) {
        return this.createError({
          path: 'ledger_type',
          message: dictionary.stakeholders.form.messages.ledgerType,
        });
      }
      return true;
    });
  ...
  const addStakeholder = useMutation<ApiResponse, ErrorResponse, FormData>({
    ...
    onSuccess: (data) => {
      toggleOpen(false);
      enqueueSnackbar(dictionary.stakeholders.form.messages.createSuccess, {
        variant: 'success',
      });
    ...
    onError: (error) => {
      ...
      if (errorMessage) {
        ...
        enqueueSnackbar(
          dictionary.stakeholders.form.errors.messages.createResponse,
          { variant: 'error' }
        );
      }
  ...
  const updateStakeholder = useMutation<ApiResponse, ErrorResponse, FormData>({
    ...
    onSuccess: (data) => {
      toggleOpen(false);
      enqueueSnackbar(dictionary.stakeholders.form.messages.updateSuccess, {
        variant: 'success',
      });
      queryClient.invalidateQueries({ queryKey: ['stakeholders'] });
    },
    onError: (error) => {
      ...
      if (validationErrors) {
        ...
        if (errorMessage) {
          ...
          enqueueSnackbar(
            dictionary.stakeholders.form.errors.messages.updateResponse,
            {
              variant: 'error',
            }
          );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} autoComplete='off' noValidate>
      <DialogTitle textAlign={'center'}>
        {!stakeholder?.id
          ? dictionary.stakeholders.form.title
          : dictionary.stakeholders.form.pageTitle.replace(
              '{name}',
              stakeholder.name
            )}
      </DialogTitle>
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <Div sx={{ mt: 1, mb: 1 }}>
              <CurrencySelector
                ...
                defaultValue={
                  stakeholder?.currency_id ?? stakeholder?.currency?.id
                }
                onChange={(newValue) => {
            ...
            <Grid size={12}>
              <Div sx={{ mt: 1, mb: 1 }}>
                <FormLabel component='legend'>
                  {dictionary.stakeholders.form.labels.ledgerType}
                </FormLabel>
                <RadioGroup
                  row
                ...
                >
                  <FormControlLabel
                    ...
                    control={<Radio />}
                    label={
                      dictionary.stakeholders.form.labels.receivableAccount
                    }
                  />
                  <FormControlLabel
                    value='payable'
                    control={<Radio />}
                    label={dictionary.stakeholders.form.labels.payableLedger}
                  />
                </RadioGroup>
                {errors.ledger_type && (
      ...
      <DialogActions>
        <Button onClick={() => toggleOpen(false)}>
          {dictionary.stakeholders.form.buttons.cancel}
        </Button>
        <LoadingButton
