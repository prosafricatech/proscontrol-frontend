...
const SaleReceiptForm: React.FC<SaleReceiptFormProps> = ({
  toggleOpen,
  sale = null,
}) => {
  ...
  const validationSchema = yup.object({
    ...
    amount: yup
      ...
      .test(
        ...
        (value) => sanitizedNumber(value) > 0
      )
      .test(
        'exceeds-receiptable-amount',
        'Entered amount exceeds receiptable amount',
        function (value) {
          const { receiptable_amount, exchange_rate } = this.parent;
          return (
            sanitizedNumber(value) * exchange_rate <= (receiptable_amount || 0)
          );
        }
      )
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField
                  ...
                  onChange={(e) => {
                    setValue(
                      'exchange_rate',
                      e.target.value ? sanitizedNumber(e.target.value) : 1,
                      {
                        shouldValidate: true,
                        shouldDirty: true,
                      }
                ...
                <TextField
                  ...
                  helperText={
                    !!errors?.amount?.message && amount > 0
                      ? `${errors.amount.message} of ${(
                          sale?.receiptable_amount || 0
