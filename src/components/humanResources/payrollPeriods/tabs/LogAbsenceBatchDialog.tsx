...
interface LogAbsenceBatchDialogProps {
  open: boolean;
  onClose: () => void;
...
const LogAbsenceBatchDialog = ({
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: any) => humanResourcesServices.addPeriodAbsenceBatch(payload),
  ...
  const validRows = rows.filter(
    (r) => r.employee && r.date && r.hours !== '' && Number(r.hours) > 0
  );
  ...
  return (
    ...
    >
      <DialogTitle>Log Absence Entries</DialogTitle>
      <DialogContent>
...
export default LogAbsenceBatchDialog;
